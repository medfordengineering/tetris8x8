...
void add_piece() {
	for (int row = 0; row < 3; row++) {
		...
		if (color == 1) red_map[row + row_count] |= (piece[row + (angle*3)] << shift_left);
		if (color == 2) green_map[row + row_count] |= (piece[row + (angle*3)] << shift_left);
	}
}
	...
	for (int row = 0; row < 8; row++) {
		if ((current_map[row+ROW_OFFSET] & (0x0FF<<COL_OFFSET)) == (0x0FF<<COL_OFFSET)) {
			for (int map_row = row+ROW_OFFSET; map_row > 0; map_row--) {
				...
				red_map[map_row] = current_map[map_row-1];
				green_map[map_row] = current_map[map_row-1];
			}
			return true;
		}
	}
	...
	for (int row = 0; row < 8; row++) {
		for (int col = 0; col < 8; col++) {
			if (current_map[row+ROW_OFFSET] & 1<<col+COL_OFFSET) {
				if (red_map[row+ROW_OFFSET] & 1<<col+COL_OFFSET) 
					matrix.drawPixel(row, col, LED_RED);  
				else if (green_map[row+ROW_OFFSET] & 1<<col+COL_OFFSET) 
